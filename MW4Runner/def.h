...
/**************************  all the Mega types  ***********************************/
 ...
 //original Multiwii Code for Mega Board
...
//  #define LEDPIN_OFF                 PORTB &= ~(1<<7);PORTC &= ~(1<<7);
  ...
  #define BUZZERPIN_PINMODE          pinMode (13, OUTPUT);pinMode (32, OUTPUT);
  #if defined PILOTLAMP
 ...
 //   #define BUZZERPIN_OFF             PORTC &= ~(1<<5);
  ...
  #define STABLEPIN_PINMODE          pinMode (31, OUTPUT);  //Left LED and GPS Indicator
  #define STABLEPIN_ON               PORTC |= 1<<6;
  #define STABLEPIN_OFF              PORTC &= ~(1<<6);
 
 //defining the Right Wing and Left Wind LED Bars for Walkera
 //RWLED = Right Wing LED, LWLED = Left Wing LED
  
  #define RWLED_PINMODE             pinMode(27, OUTPUT);
  #define RWLED_TOGGLE              PINA |= (1<<5);
  #define RWLED_ON                  PORTA |= (1<<5);
  #define RWLED_OFF                 PORTA &= ~(1<<5);  
  #define LWLED_PINMODE             pinMode(26, OUTPUT);
  #define LWLED_TOGGLE              PINA |= (1<<4);
  #define LWLED_ON                  PORTA |= (1<<4);
  #define LWLED_OFF                 PORTA &= ~(1<<4);         
  ...
  //Walkera remapping to the Advance Runner pins
  #define PITCHPIN                   0  //PIN 62 =  PIN A8
...
//please submit any correction to this list.
 ...
 /* These felt good but Roll control reversed
  * #define ACC_ORIENTATION(X, Y, Z)  {imu.accADC[ROLL]  = -X; imu.accADC[PITCH]  =  -Y; imu.accADC[YAW]  =  Z;}
  *#define GYRO_ORIENTATION(X, Y, Z) {imu.gyroADC[ROLL] = Y; imu.gyroADC[PITCH] = -X; imu.gyroADC[YAW] = -Z;}
  *#define MAG_ORIENTATION(X, Y, Z)  {imu.magADC[ROLL]  =  -X; imu.magADC[PITCH]  =  Y; imu.magADC[YAW]  = Z;}   //
  *
  */
  #undef INTERNAL_I2C_PULLUPS
  //
  //motors are mapped to servos as follows
  #define SERVO_5_PINMODE           pinMode(6,OUTPUT);                      // BI LEFT
  #define SERVO_5_PIN_HIGH          PORTH |= 1<<3;
  #define SERVO_5_PIN_LOW           PORTH &= ~(1<<3);
  #define SERVO_6_PINMODE           pinMode(2,OUTPUT);                      // TRI REAR - BI RIGHT
